...
IO_Stream *IO_ReadStream(const char *file) {
  ...
  if (stream == NULL) {
    IO_Log("IO_ReadStream: cannot open %s\n", file);
    return NULL;
  }
  return stream;
}
...
int IO_ReadU8(IO_Stream *stream, Uint8 *value) {
  ...
  if (!IS_HEX(cu) || !IS_HEX(cl)) {
    IO_Log("IO_ReadU8: bad hex (%c%c) at pos %d\n", cu, cl, IO_Offset(stream));
    return 2;
  }
...
Sint32 IO_Offset(IO_Stream *stream) { return SDL_RWtell(stream); }

int IO_QuitStream(IO_Stream *stream) { return SDL_RWclose(stream); }